...
/**
 * ****************************************************************************
 * To set adc map range from : 0v ~ 3.0v (default) or 0v ~ 3.3v
 *
 * @return
 * none
 *
 * @parameter
 * p_adc_channel_x, ADC_Channel_0 | ADC_Channel_1 | ADC_Channel_2 |
 *                  ADC_Channel_3 | ADC_Channel_4 | ADC_Channel_5 |
 *                  ADC_Channel_6 | ADC_Channel_7
 *
 * p_adc_volt_range : 0 (default)- 0v ~ 3.0v map to 0h ~ 3FFh
 *					  Others     - 0v ~ 3.3v map to 0h ~ 3FFh
 *
 *
 * ****************************************************************************
 */
void ADC_Set_Voltage_Map_Range(BYTE p_adc_channel_x,BYTE p_adc_volt_range)
{
	BYTE l_adc_channel_index;
    ...
    for(l_adc_channel_index=0x00;
        ...
        l_adc_channel_index++)
    {
		if((p_adc_channel_x&(0x01<<l_adc_channel_index)))
		{
			if(p_adc_volt_range==0) 
			{
				CLEAR_MASK(ADCIVMFSCS1,(0x01<<l_adc_channel_index));				
			}
			else SET_MASK(ADCIVMFSCS1,(0x01<<l_adc_channel_index));
		}
	}
}
