...
*
* COPYRIGHT LCFC 2018 ALL RIGHTS RESERVED
*
...
void DetectAdaptorWatt()
{
    ...
    if(TypeCAdpPdStableCheck != 0)
    {
        ...
        if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_RX_SOURCE_CAPABILITIES, SMB3_DATA))
        {
            TypeCCommFailCnt++;
        ...
        if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_ACTIVE_CONTRACT_PDO, SMB3_DATA))
        {
            TypeCCommFailCnt++;
            TypeCAdpPdStableCheck--;
        }
        else
        {
	      ...
	      if(Port1CurrentTypeCWatt < 16)
            {
                TypeCAdpPdStableCheck--; // David+ for BAT2 Event lost
            }
            else
            {
            ...
            else
            {
                ...
                //A485D00080---S
               // UnknowAdpRetry++;//A485D000145:
                if(UnknowAdpRetry <= 5)  //A485D000145:3->5
                {
	              UnknowAdpRetry++;  //A485D000145+:
                ...
                {  
                    ...
                    //A485D000151+
                    if(IS_MASK_SET(TypeC_LenovoAdaptor,Port1_IslenovoAdaptor) && (TypeC_LenovoAdaptor_P1HWResetTimes < 3))
                  	{//A485D000151+
                       if(TypeC_LenovoAdaptor_P1HWResetTimes < 3)
                        {
                            TypeC_LenovoAdaptor_P1HWResetTimes ++;
                            RamDebug(0x85);                       
                            CLEAR_MASK(TypeCPort2Status,TypeCIniOK);
                            Send4ccCmd(TPS65988_4CC_CMD_HARDWARE_RESET, TYPECPort1);
                        }
     		      }else{//A485D000151+
    			    TypeCAdpDetectDelay = 0; //A485D000145+:
                        TypeCAdpPdStableCheck = 0;//A485D000145+:
                        TypeC_LenovoAdaptor_P1HWResetTimes = 0;
...
//A485D00080---S
    ...
    if(TypeCPort2AdpPdStableCheck != 0)
    {
        ...
        if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_RX_SOURCE_CAPABILITIES, SMB3_DATA))
        {
        ...
        if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_ACTIVE_CONTRACT_PDO, SMB3_DATA))
        {
            TypeCPort2CommFailCnt++;
            TypeCPort2AdpPdStableCheck--;
        }
        else
        {
	     ...
	     if(Port2CurrentTypeCWatt < 16)
           {
              TypeCPort2AdpPdStableCheck--; // David+ for BAT2 event lost
	      }
	    else
            {
            ...
            else
            {
                ...
                //A485D00080---S
                //Port2UnknowAdpRetry++;//A485D000145:
                if(Port2UnknowAdpRetry <= 5)  //A485D000145 3->5:
                {
                    Port2UnknowAdpRetry++;  //A485D000145+:
                ...
                else
                {
                   ...
                   if(IS_MASK_SET(TypeC_LenovoAdaptor,Port2_IslenovoAdaptor) && (TypeC_LenovoAdaptor_P2HWResetTimes < 3))
                  	{//A485D000151+
			          if (TypeC_LenovoAdaptor_P2HWResetTimes < 3)
			           {
    			           TypeC_LenovoAdaptor_P2HWResetTimes ++;
                            RamDebug(0x86);
                            CLEAR_MASK(TypeCPort2Status,TypeCIniOK);
			        ...
			        {//A485D000151+
                        ...
                        TypeCPort2AdpPdStableCheck =0;
                        TypeC_LenovoAdaptor_P2HWResetTimes = 0;
                        //A485D000145+--->E:
                        #if Support_Lenovo_P2P_V2P0
                        //A485D000116: Add unsupport adapter process,set flag for bios,close ac in
            ...
            if(isBatteryExist())
            {
                SetMultiPortSNK(TYPECPort2); //A485D000125 fix cs18 dock led blink.
            }
            //Re-setting adaptor parameter
            //ADPID_ON_EN();
            //CLEAR_MASK(pEdgeProjectFlag1,pEdgeADPIDReady);
    ...
    if(TypeCIniDelay == 0)
    {
        switch(TypeCProcessStep)
        {
            case TPS65988_READ_MODE:
                //Get PD mode
                if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_MODE, SMB3_DATA))
                {
                    TypeCCommFailCnt++;
                    TypeCIniDelay = 100;
            ...
            case TPS65988_READ_BOOT_FLAGS_AND_OTP_CONFIG:
                ...
                if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_BOOT_FLAG_OTP_CONFIG, SMB3_DATA))
                {
                    TypeCCommFailCnt++;
            ...
            case TPS65988_INIT_EVENT_MASK_READ_TO_GET_SIZE:
                ...
                **************************************************
                */                
                if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_INT_MASK2, SMB3_DATA))
                {
                    TypeCCommFailCnt++;
                    if(TypeCCommFailCnt == 3)
                    {
            ...
            case CHECK_POWER_STATUS:
                ...
                if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_POWER_STATUS, SMB3_DATA))
                {
                    TypeCCommFailCnt++;
                    if(TypeCCommFailCnt == 3)
                    {
   ...
   // BYTE TempRomData, Temp8;
    ...
    if(TypeCPort2IniDelay == 0)
    {
        switch(TypeCPort2ProcessStep)
        {
            case TPS65988_READ_MODE:
                //Get PD mode
                if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_MODE, SMB3_DATA))
                {
                    TypeCPort2CommFailCnt++;
            ...
            case TPS65988_READ_BOOT_FLAGS_AND_OTP_CONFIG:
                //Read Boot Flags and OTP configuration register first
                if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_BOOT_FLAG_OTP_CONFIG, SMB3_DATA))
                {
                    TypeCPort2CommFailCnt++;
            ...
            case TPS65988_INIT_EVENT_MASK_READ_TO_GET_SIZE:
                ...
                **************************************************
                */
                if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_INT_MASK2, SMB3_DATA))
                {
                    TypeCPort2CommFailCnt++;
                    if(TypeCPort2CommFailCnt == 3)
                    {
            ...
            case CHECK_POWER_STATUS:
                //Check power status for dead battery
                if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_POWER_STATUS, SMB3_DATA))
                {
                    TypeCPort2CommFailCnt++;
                    if(TypeCPort2CommFailCnt == 3)
                    {
...
BOOL TypecPDProcess(BOOL verify)
{
    ...
    {
        //Get Event number
        if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_INT_EVENT2, SMB3_DATA))
        {
            TypeCCommFailCnt++;
            if(TypeCCommFailCnt == 3)
            {
...
//T495XU0002: S+  enable P2P function
#if !Support_Lenovo_P2P_V2P0
                    if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_CONTROL_CONFIG, SMB3_DATA_TEMP1))
                    {
...
//T495XU0002: E+
                ...
                case TPS65988_INT_PR_SWAP_Num:
                    ...
                    if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_POWER_STATUS, SMB3_DATA_TEMP))
                    {
                ...
                case TPS65988_INT_DR_SWAP_Num:
                    ...
                    if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_STATUS, SMB3_DATA_TEMP))
                    {
                ...
                case TPS65988_INT_VDM_RECEIVED_Num:
                    //A Vendor Defined Message has been received.
                    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_RX_VDM, SMB3_DATA))
                    {
                        TypeCCommFailCnt++;
                    }
                    else
                ...
                case TPS65988_INT_NEW_CONTRACT_AS_CONS_Num:
                    ...
                    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_ACTIVE_CONTRACT_RDO, SMB3_DATA_TEMP))
                    {
                        TypeCCommFailCnt++;
                    }
                    else
...
//T495XU0002: E+
                ...
                case TPS65988_INT_SOURCE_CAP_MSG_READY_Num:
                    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_RX_SOURCE_CAPABILITIES, SMB3_DATA_TEMP))
                    {
                        TypeCCommFailCnt++;
                    }
                    else
...
//T495XU0002: E+
                            ...
                            {
                              TypeCAdpPdStableCheck = 10;//A485D000145+:
                            //A485D000116: Add unsupport adapter process,set flag for bios,close ac in
                              //  SinkPowerPathControl(0, TYPECPort1); //A485D000145-:
                                //set unsupport adapater flag
                             //   SET_MASK(u4SenseSTS3, UnSupportAdapterPort1);//A485D000145-:
                            }
                        }
                        else
                        {
                            //Dis-connection
                            UnknowAdpRetry=0;//A485D000145+
                            CLEAR_MASK(TypeC_LenovoAdaptor, Port1_IslenovoAdaptor);//A485D000151+
                            TypeC_LenovoAdaptor_P1HWResetTimes = 0;//A485D000151+
                            CLEAR_MASK(TypeCStatus,TypeCAdpExist);
                            CLEAR_MASK(TypeCStatus,TypeCAdpReady);
...
//T495XU0002: E+
                ...
                case TPS65988_INT_DATA_STATUS_UPDATE_Num:
                    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_DATA_STATUS, SMB3_DATA_TEMP))
                    {
                        TypeCCommFailCnt++;
                    }
                    else
                ...
                case TPS65988_INT_USER_VID_ALT_MODE_ATTN_VDM_Num:
                    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_RX_USER_VID_ATTENTION_VDM, SMB3_DATA))
                    {
                        TypeCCommFailCnt++;
                    }
                    else
                ...
                case TPS65988_INT_USER_VID_ALT_MODE_OTHER_VDM_Num:
                    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_RX_USER_VID_OTHER_VDM, SMB3_DATA))
                    {
                        TypeCCommFailCnt++;
                    }
                    else
...
BOOL TypecPDPort2Process(BOOL verify)
{
    ...
    {
        //Get Event number
        if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_INT_EVENT2, SMB3_DATA))
        {
            TypeCPort2CommFailCnt++;
            if(TypeCPort2CommFailCnt == 3)
            {
...
//T495XU0002: S+  enable P2P function
#if !Support_Lenovo_P2P_V2P0
                    if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_CONTROL_CONFIG, SMB3_DATA_TEMP1))
                    {
                        //RamDebug(SMB3_DATA_TEMP1[1]);                        
                        SMB3_DATA_TEMP1[1] = (SMB3_DATA_TEMP1[1]&0x7F);
                        //PD Controller does not automatically initiate and send swap to DFP requests to the farend device.
...
//T495XU0002: E+
                ...
                case TPS65988_INT_PR_SWAP_Num:
                    //A Power role swap has completed.
                    if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_POWER_STATUS, SMB3_DATA_TEMP))
                    {
                        if(TPS65988_POWER_STATUS_SOURCE_SINK(SMB3_DATA_TEMP[0]))
                        {
                            //Connection provides power (PD Controller as sink).
                    ...
                    break;
                case TPS65988_INT_DR_SWAP_Num:
                    if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_STATUS, SMB3_DATA_TEMP))
                    {
                    ...
                    break;
                case TPS65988_INT_VDM_RECEIVED_Num:
                    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_RX_VDM, SMB3_DATA))
                    {
                        TypeCPort2CommFailCnt++;
                    }
                    else
                ...
                case TPS65988_INT_NEW_CONTRACT_AS_CONS_Num:
                    //An RDO from the far-end device has been accepted and the PD Controller is a Sink.
                    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_ACTIVE_CONTRACT_RDO, SMB3_DATA_TEMP))
                    {
                        TypeCPort2CommFailCnt++;
                    }
                    else
...
//T495XU0002: E+              
                ...
                case TPS65988_INT_NEW_CONTRACT_AS_PROV_Num:
                    ...
                    break;
                case TPS65988_INT_SOURCE_CAP_MSG_READY_Num:
                    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_RX_SOURCE_CAPABILITIES, SMB3_DATA_TEMP))
                    {
                        TypeCPort2CommFailCnt++;
                    }
                    else
...
//T495XU0002: E+                    
                            ...
                            {
                              TypeCPort2AdpPdStableCheck = 10;//A485D000145:+
                                //RamDebug(0x83);
                                //A485D000116: Add unsupport adapter process,set flag for bios,close ac in
                               // SinkPowerPathControl(0, TYPECPort2);//A485D000145-:
                                //set unsupport adapater flag
                                //SET_MASK(u4SenseSTS3, UnSupportAdapterPort2);//A485D000145-:
                            }
                        }
                        else
                        {
                            ...
                            CLEAR_MASK(TypeC_LenovoAdaptor, Port2_IslenovoAdaptor);//A485D000151+
                            TypeC_LenovoAdaptor_P2HWResetTimes = 0;//A485D000151+
                            CLEAR_MASK(TypeCPort2Status,TypeCAdpExist);
                            CLEAR_MASK(TypeCPort2Status,TypeCAdpReady);
...
//T495XU0002: E+
                    ...
                    break;
                case TPS65988_INT_DATA_STATUS_UPDATE_Num:
                    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_DATA_STATUS, SMB3_DATA_TEMP))
                    {
                        TypeCPort2CommFailCnt++;
                    }
                    else
                ...
                case TPS65988_INT_STATUS_UPDATE_Num:
                    ...
                    else
                    {
                        ...
                        {
                          //A485D000143--->Start
                               Port2UnknowAdpRetry=0;//A485D000145:+
                            CLEAR_MASK(TypeCPort2Status,TypeCAdpExist);
                            CLEAR_MASK(TypeCPort2Status,TypeCAdpReady);
                            CLEAR_MASK(TypeCPort2Status2,TypeC_Docking_Attached);
                           //A485D000116: Add unsupport adapter process,set flag for bios,close ac in
                ...
                case TPS65988_INT_CMD2_COMPLETE_Num:
                    ...
                    break;
                case TPS65988_INT_USER_VID_ALT_MODE_ENTERED_Num:
			SET_MASK(TypeC_LenovoAdaptor,Port2_IslenovoAdaptor);//A485D000151+ 180615
                    SET_MASK(TypeCPort2Status,TypeCProcessOK);
                    break;
                case TPS65988_INT_USER_VID_ALT_MODE_EXITED_Num:
                    SET_MASK(TypeCPort2Status,TypeCProcessOK);
                    break;
                case TPS65988_INT_USER_VID_ALT_MODE_ATTN_VDM_Num:
                    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_RX_USER_VID_ATTENTION_VDM, SMB3_DATA))
                    {
                        TypeCPort2CommFailCnt++;
                    }
                    else
                    ...
                    break;
                case TPS65988_INT_USER_VID_ALT_MODE_OTHER_VDM_Num:
                    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_RX_USER_VID_OTHER_VDM, SMB3_DATA))
                    {
                        TypeCPort2CommFailCnt++;
                    }
                    else
...
void OEM_TYPE_C_Hook_PDMonitor(void)
{
    SMB3_DATA[0] = 0x00;
    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_MODE, SMB3_DATA))
    {
    ...
    if(Port1MonitorFailCnt >= 5)
    {
        ...
        RamDebug(0x7A);
        Send4ccCmd(TPS65988_4CC_CMD_COLD_BOOT, TYPECPort1);
    }
    ...
    if(Port2MonitorFailCnt >= 5)
    {
        ...
        RamDebug(SMB3_DATA[2]);
        RamDebug(0x8A);
        Send4ccCmd(TPS65988_4CC_CMD_COLD_BOOT, TYPECPort2);
    }
}
#endif
/*
...
#if Support_SecBattery
                ||(IS_MASK_SET(SecBattSTS,BattExist))
#endif
                ) //A485D000102+  //Fix AC only cannot boot up
                {
                    //20V->9V
                    RamDebug(0x99);
...
{
    //A485D000130: Add PD control for i2c retry--ti spec start
    Send4ccCmd(TPS65988_4CC_CMD_RETRY_I2C, TYPECPort1);
    //Send4ccCmd(TPS65988_4CC_CMD_RETRY_I2C, TYPECPort2);//A485D000148-
    //A485D000130: Add PD control for i2c retry--ti spec end
...
//A485D000154:+---->End
    ...
    //A485D000148+ 180613 for 4CC CMD delay --->Start
    do
    {
        if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, tmpTYPECAddr, tmpTYPCCmd, &SMB3_DATA[0]))
        {
            Delay1MS(5);
            RamDebug(0xE5);
        }
    ...
    if(portID == TYPECPort1)
    {
        ...
        else
        {
            ...
            if(OnOff)
            {
                //Turn on Type-C VBus power
//T495XU0002: S+  enable P2P function
                if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_01_Addr, TPS65988_GLOBAL_SYSTEM_CONFIG, SMB3_DATA_TEMP))
                {
                    RamDebug(0xBB);
    ...
    else if(portID == TYPECPort2)
    {
        ...
        else
        {
            ...
            if(OnOff)
            {
                //Turn on Type-C VBus power
//T495XU0002: S+  enable P2P function
                if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TypeC_02_Addr, TPS65988_GLOBAL_SYSTEM_CONFIG, SMB3_DATA_TEMP))
                {
                    RamDebug(0xBC);
                    RamDebug(SMB3_DATA_TEMP[4]);
                    //011b:PP3 configured for Sink (input), but will wait for SYS_RDY command(SRDY) before closing the switch.
            ...
            */
            ...
            switch (cmd)
            {
                ...
                case USB_PDC_REQ_GET_TX_SOURCE_PDO:
                    ...
                    RamDebug(0xFE);
#endif
                    if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_TX_SOURCE_CAPABILITIES, SMB3_DATA))
                    {
#if DEBUG_UCSI
                        RamDebug(SMB3_DATA[0]);
                        RamDebug(0xEF);
                ...
                case USB_PDC_REQ_GET_TX_SINK_PDO:
                    ...
                    RamDebug(0xFD);
#endif
                    if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_TX_SINK_CAPABILITIES, SMB3_DATA))
                    {
#if DEBUG_UCSI
                        RamDebug(SMB3_DATA[0]);
                        RamDebug(0xDF);
                ...
                case USB_PDC_REQ_OPERATE_AS_DFP:
                    if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_CONTROL_CONFIG, SMB3_DATA))
                    {
                ...
                case USB_PDC_REQ_GET_ALTERNATE_MODES:
                    ...
                    if(TempValue == 0)
                    {
                        ...
                        #else
                        if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_USER_SID_STATUS,SMB3_DATA))
                        {
                            if(SMB3_DATA[0] != 0x00)
                            {
                                ...
                                if(bWSMBusBlock(chSMB_TYPEC, SMbusWBK, TempAddr, TempTypeData, &SMB3_DATA_TEMP[0], 2, SMBus_NoPEC))
                                {
                                    ...
                                    {
                                        if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TempTypeData,SMB3_DATA_TEMP))
                                        {
                        ...
                        if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_DP_SID_STATUS, SMB3_DATA))
                        {
                            if(SMB3_DATA[0] != 0x00)
                            {
                    ...
                    else
                    {
                        ...
                        if(bWSMBusBlock(chSMB_TYPEC, SMbusWBK, TempAddr, TempTypeData, &SMB3_DATA[0], 5, SMBus_NoPEC))
                        {
                            ...
                            {
                                if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_RX_VDM,SMB3_DATA))
                                {
                                    /*i = SMB3_DATA[0]&0x07;
                ...
                case USB_PDC_REQ_GET_CURRENT_CAM:
                    ...
                    if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_INTEL_VID_STATUS, SMB3_DATA))
                    {
                        if(SMB3_DATA[0] != 0x00)
                        {
                            Ucsi_message_in[0]++;
                    ...
                    if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_TI_VID_STATUS, SMB3_DATA))
                    {
                        if(SMB3_DATA[0] != 0x00)
                        {
                            Ucsi_message_in[0]++;
                ...
                else
                {
                    ...
                    switch (request_packet->request)
                    {
                        case USB_PDC_REQ_HARD_RESET:
                            ...
                            {
                                if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_STATUS, SMB3_DATA_TEMP1))
                                {
                                    if(usb_pdc_id == 0)
                                    {
                        ...
                        case USB_PDC_REQ_GET_RX_SOURCE_PDO:   //RX:Device;TX:PDC
                            ...
                            break;
                        case USB_PDC_REQ_GET_RX_SINK_PDO:
                            if(bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_RX_SINK_CAPABILITIES, SMB3_DATA))
                            {
                                /*request_packet->pdos_valid      = SMB3_DATA[0];
                                for(i = 0;(i < request_packet->pdos_valid)&&(i < 4); i++)
                                {
...
void UsbPdcGetPowerRole(BYTE usb_pdc_id)
{
    ...
    {
        //  if (IS_MASK_SET(TypeCStatus,TypeCIniOK)) {
        if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_POWER_STATUS, SMB3_DATA_TEMP))
        {
            TypeCCommFailCnt++;
        }
        else
...
void UsbPdcGetDataRole(BYTE usb_pdc_id)
{
    ...
    {
        //if (IS_MASK_SET(TypeCStatus,TypeCIniOK)) {
        if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_STATUS, SMB3_DATA_TEMP))
        {
            TypeCCommFailCnt++;
        }
        else
...
void SendPDO20VTo9V(BYTE portNum)
{
    ...
    else if(portNum == TYPECPort2)
    {
        ...
        return;
    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, tmpAddr, TPS65988_TX_SINK_CAPABILITIES, &SMB3_DATA[0]))
    {
        //TypeCCommFailCnt++;
        tmpCOMFailFlag = 1;
    }
...
void PDOPortSwitchToNormal(void)
{
    ...
    //if(IS_MASK_SET(TypeCPort2Status,TypeCAdpExist)&&IS_MASK_SET(TypeCPort2Status,TypeCAdpReady))--mark 20180418 for cs18 dock maybe canot identify adapter
    {
        ...
        {
            RamDebug(0xA2);
            RamDebug(Port2CurrentRDO);
            RamDebug(Port2ObjectPosition20V);
            RamDebug(0x2A);
...
BOOL isBatteryExist(void)
{
    return (((TRUE == BattExistGet)/* && (1 == M_BATdata_ready))  \
            ||  ((TRUE == SecBattExistGet) && (1 == S_BATdata_ready)*/)); //A485D000112:fix dual-battery shipmode ec hangup with black screen
}
...
void TPS65988_UcsiRelativeInit(void)
{
    if(isBatteryExist())
    {
        ...
        RemainCap1 = ((nRemainingCapH << 8) + nRemainingCapL);
        FullChgCap1 = ((nFullChgCapH << 8) + nFullChgCapL);
#if Support_SecBattery
        RemainCap2 = ((SecRemainCapH << 8) + SecRemainCapL);     
        FullChgCap2 = ((SecFullChgCapH << 8) + SecFullChgCapL);
#endif
        if(0 != (FullChgCap1 + FullChgCap2))//A485D000112:fix dual-battery shipmode ec hangup with black screen
        {
            RelativeStateOfChg = ((RemainCap1 + RemainCap2)*100)/(FullChgCap1 + FullChgCap2);
            UCSI_RelativeStateOfChg = (BYTE)RelativeStateOfChg;
        ...
        if(IS_MASK_SET(TypeCStatus, TypeCAdpReady) && IS_MASK_SET(TypeCPort2Status, TypeCAdpReady))
        {
            ...
            if(isNeedSwitch)
            {
                if((Port2CurrentTypeCWatt > Port1CurrentTypeCWatt)
                    && (Port_Select != TYPECPort2))   //A485D000156+
                {
                    Port_Select         = TYPECPort2;
                    TypeCAdpID      = TypeCPort2AdpID;
                    CurrentTypeCWatt    = Port2CurrentTypeCWatt;
                    ...
                    && (TYPECPort1 != Port_Select)) //A485D000156+
                {
                    Port_Select         = TYPECPort1;
                    TypeCAdpID      = TypeCPort1AdpID;
                    CurrentTypeCWatt    = Port1CurrentTypeCWatt;
...
void SetMultiPortSNK(BYTE portNum)
{
    ...
    if(!bRSMBusBlock(chSMB_TYPEC, SMbusRBK, tmpAddr, TPS65988_GLOBAL_SYSTEM_CONFIG, &SMB3_DATA[0]))
    {
        tmpCOMFailFlag = 1;
    }
    else
