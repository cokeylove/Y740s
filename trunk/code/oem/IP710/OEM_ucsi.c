...
RamDebug(0xD6);
                 ...
                 //connector = SysConvertUsbConnectorIdToUsbPdcId(connector);
                 if (0 <= connector){
                    bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_SYSTEM_CONFIG, SMB3_DATA_TEMP);
                    Ucsi_message_in[0] = 0;
                    Ucsi_message_in[1] = 0;
                  ...
                  //    Ucsi_message_in[0] |= UCSI_CONNECTOR_SUPPORTS_DRP;
                   ...
                   // Ucsi_message_in[0] |= UCSI_CONNECTOR_SUPPORTS_DRP;//A485D00096   //Yanis Modify for UCSI WHLK Test fail 180411
                  ...
                  //  else if(((SMB3_DATA_TEMP[0] & 0x07) == 0x00) || (SMB3_DATA_TEMP[0] & 0x07) == 0x01)
                     //   Ucsi_message_in[0] |= UCSI_CONNECTOR_SUPPORTS_UFP_ONLY;//A485D00096  //Yanis Modify for UCSI WHLK Test fail 180411
                  ...
                  //    Ucsi_message_in[0] |= UCSI_CONNECTOR_SUPPORTS_USB_3;
                    ...
                    bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_USER_VID_CONFIGURATION, SMB3_DATA_TEMP);
                    if((SMB3_DATA_TEMP[0] & BIT0) == BIT0)
                        Ucsi_message_in[0] |= UCSI_CONNECTOR_SUPPORTS_ALTERNATE_MODE;
                    bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_DP_SID_CONFIGURATION, SMB3_DATA_TEMP);
                    if((SMB3_DATA_TEMP[0] & BIT0) == BIT0)
                        Ucsi_message_in[0] |= UCSI_CONNECTOR_SUPPORTS_ALTERNATE_MODE;
                    bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_INTEL_VID_CONFIGURATION, SMB3_DATA_TEMP);
                    if((SMB3_DATA_TEMP[0] & BIT0) == BIT0)
                        Ucsi_message_in[0] |= UCSI_CONNECTOR_SUPPORTS_ALTERNATE_MODE;
                    bRSMBusBlock(chSMB_TYPEC, SMbusRBK, TempAddr, TPS65988_TI_VID_CONFIGURATION, SMB3_DATA_TEMP);
                    if((SMB3_DATA_TEMP[0] & BIT0) == BIT0)
                        Ucsi_message_in[0] |= UCSI_CONNECTOR_SUPPORTS_ALTERNATE_MODE;
#if DEBUG_UCSI
