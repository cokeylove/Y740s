...
BYTE scan_keys(void)
{
    ...
    {
      //  return(0x01); //HEGANG0011: remove check keyscan buffer half-full
    }
...
void Init_Kbd(void)
{
	//CheckKSO1617Support(); //REJERRY012:remove.
...
void service_scan(void)
{
	...
	//XITING0045:S Fix when ECFlash press kb ec reset
	if(IS_MASK_SET(SYS_MISC1, b3_KB_Disable))
	{
		return;
	}
	//XITING0045:E

	//Enable_Any_Key_Irq(); //REJERRY043:add.  //REJERRY045:remove.
    ...
    /* There is scan activity. */
  	if (scan_activity)
    {  
    	//REJERRY013:S+ add system wake up from S3 by internal keyboard.
    	if (SystemIsS3)
		{
			if (KB_S3Dly > 10)
			{
		...
		//REJERRY013:E+ add system wake up from S3 by internal keyboard.
	}
    /* No scan activity */
    else
    {
    	KB_S3Dly = 0; //REJERRY013:add.
...
#else	// no keyboard	// Alex0429 modify to null function.
...
void Clear_Typematic(void){}
void Lock_Scan(void){}
//BROOKEY055: Start add sending scan code for button test 
//FLAG Buffer_Key(BYTE row_column){ return TRUE;}
FLAG Buffer_Key(BYTE row_column)
{
    /* Ready for successful operation */
    FLAG ready = TRUE;
...
void Enable_Any_Key_Irq(void){}
//BROOKEY055: Start add sending scan code for button test 
//BYTE Get_Buffer(void){  return 0x00; }
BYTE Get_Buffer(void)
{
    BYTE buffer_data;
...
//BROOKEY055:End
BYTE PS2_Scan_Code_To_USB_HID(void){ return 0x00; }
