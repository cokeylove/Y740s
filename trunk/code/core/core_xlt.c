...
void Send_Key(BYTE table_entry, BYTE event)
{
    ...
    if(table_entry >= SSKEY2_OVL_CODE)
    {
		...
		//REJERRY027:remove hotkey enable table.
		//REJERRY004: S+ add hotkey enable table.
        /*if(IS_MASK_CLEAR(ACPI_HOTKEY, HotkeyDisable))
		{
        		table_entry = sskey2_overlay_table1[temp];	
		}
		else
		{
		...
		table_entry = sskey2_overlay_table[temp]; //REJERRY027:default if hotkey disable table.
...
void sskey3_proc(BYTE sskey2, BYTE event)
{
	...
	//XITING0007:S+ 
	if((sskey2==0x82) && (event==MAKE_EVENT))
	{
		SET_MASK(WinKEY_STATE, WinKEY_STATE_Check);
	}else if((sskey2==0x82) && (event==BREAK_EVENT))
	{
		CLEAR_MASK(WinKEY_STATE, WinKEY_STATE_Check);
	}
	//XITING0007:E+ 

	//REJERRY076: S+ add win key enable/diable process.
	if(IS_MASK_SET(GameZoneState, winkeydisable))
	{
		if((sskey2==0x82) && (event==BREAK_EVENT))
		{
			if(IS_MASK_SET(WinKEY_STATE, WinKey_Sended))
			{
				winkey_proc(0x82, BREAK_EVENT);
			}
			CLEAR_MASK(WinKEY_STATE, WinKey_Pressed);
			CLEAR_MASK(WinKEY_STATE, WinKey_Sended);
			return;
		}
		if(IS_MASK_SET(WinKEY_STATE, WinKey_Pressed)&&(sskey2!=0x82) && (event==MAKE_EVENT))
		{
			winkey_proc(0x82, MAKE_EVENT);
			CLEAR_MASK(WinKEY_STATE, WinKey_Pressed);
			SET_MASK(WinKEY_STATE, WinKey_Sended);
		}
		if(IS_MASK_CLEAR(WinKEY_STATE, WinKey_Pressed) && (sskey2==0x82) && (event==MAKE_EVENT))
		{
			SET_MASK(WinKEY_STATE, WinKey_Pressed);
			return;
		}
	}
	//REJERRY076: E+.
...
//REJERRY076: S+ add win key process function.
void winkey_proc(BYTE sskey2, BYTE event)
{
    BYTE code_byte;
    BYTE index;
...
void special_code(BYTE code_byte, BYTE event)
{
    ...
    //const CBYTE key126_ctrl_mk[]   = {0xE0,0x7E,0xE0,0xF0,0x7E,0x00};						//XITING0034:fix break(Fn+b) not Correct
	const CBYTE key126_ctrl_mk[]   = {0x14,0xE0,0x7E,0xE0,0xF0,0x7E,0xF0,0x14,0x00};	//XITING0034:fix break(Fn+b) not Correct
    ...
    else
    {
        if(event == MAKE_EVENT)
        {
            ...
            {
				if(code_byte == 1) Buffer_String(key126_ctrl_mk);
                else
				{
